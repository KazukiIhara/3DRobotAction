...
void RenderController::ApplyPostEffect() {
	...
	for (uint32_t i = 0; i < currentFramePostEffectNum; i++) {
		...
		case PostEffectType::DepthNormalOutline:
			DrawRenderTextureWithParamaterDepthAndNormal(commandList, command);
			break;
		}
...
void RenderController::DrawRenderTextureWithParamaterDepthAndNormal(ID3D12GraphicsCommandList* commandList, const PostEffectCommand& command) {
	// 現在の書き込み先のレンダーテクスチャを切り替え
	currentRenderTarget_ = colorPostEffectRenderTexture_[currentColorPostEffectRenderTextureIndex_].get();
	// 書き込み可能状態にする
	currentRenderTarget_->TransitionToWrite();
